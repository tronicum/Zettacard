...
| **1.1 — the audio add-on** | Narration of every question and answer, shipped as a **separate downloadable pack** per module and locale, never bundled — Angelschein alone is ~16 h and ~174 MB across 15 locales, and that is one module of 29. Questions first, lessons later. Scoping and the measured language coverage are in `zettacard-kb/docs/audio-narration-scoping.md`; ADR-0008 (subtitles, narration, accessibility) is owed. Note: Ollama cannot do this — it has no TTS model — so the tool is Piper, which covers 16 of our 18 locales. |
